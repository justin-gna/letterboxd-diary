...
import styles from "./LetterboxdDiary.module.css";
...
export interface LetterboxdDiaryProps {
  /** Full URL of the deployed Cloudflare Worker */
  apiUrl: string;
  /** Display name used in the header and footer, e.g. "Justin" */
  name: string;
  count?: number;
  minRating?: number;
  reviewsOnly?: boolean;
...
function SkeletonCard() {
  return (
    <div className={styles.skeleton} data-testid="skeleton-card">
...
export function LetterboxdDiary({
  apiUrl,
  name,
  count = 6,
  minRating,
  ...
  layout = "grid",
  showReviews = true,
}: LetterboxdDiaryProps) {
  const safeCount = Number.isFinite(count) ? Math.max(1, Math.round(count)) : 6;

  const { entries, isLoading, error, username, refresh } = useLetterboxdDiary({
    apiUrl,
    count: safeCount,
    minRating,
    reviewsOnly,
  ...
  // Observe wrapper width to drive responsive carousel per-page count
  useEffect(() => {
    ...
    const update = (width: number) => {
      if (width >= 720) setPerPage(4);
      else if (width >= 560) setPerPage(3);
      else if (width >= 380) setPerPage(2);
      else setPerPage(1);
    };
  ...
  // Reset to first page when perPage or entries change
  useEffect(() => {
    setCarouselPage(0);
  }, [perPage, entries]);
    ...
    return (
      <div className={styles.wrapper}>
        <div className={styles.error}>
          ...
          <a
            href="https://github.com/justin-gna/letterboxd-diary-worker"
            target="_blank"
  ...
  const pageEntries = entries.slice(carouselPage * perPage, (carouselPage + 1) * perPage);

  const renderCards = (items: DiaryEntry[]) =>
    items.map((entry) => (
      <CardErrorBoundary key={entry.entryUrl}>
        <DiaryEntryCard entry={entry} showReview={showReviews} layout={layout} />
      </CardErrorBoundary>
    ));
  ...
  if (isLoading) {
      ...
      content = (
        <div className={styles.carouselWrapper}>
          <div
            ...
            style={{ gridTemplateColumns: `repeat(${Math.min(safeCount, perPage)}, 1fr)` }}
          >
            {Array.from({ length: Math.min(safeCount, perPage) }).map((_, i) => (
              <SkeletonCard key={i} />
            ))}
          </div>
        </div>
    ...
    content = (
      <div className={styles.error}>
        <p>Could not reach the Worker. Make sure it is deployed and <code>ALLOWED_ORIGIN</code> includes your domain.</p>
        <a
          href="https://github.com/justin-gna/letterboxd-diary-worker"
          target="_blank"
          rel="noopener noreferrer"
    ...
    content = <div className={styles.empty}>No diary entries found.</div>;
    ...
    <div className={styles.wrapper} ref={wrapperRef}>
      <div className={styles.header}>
        ...
        <a href="https://letterboxd.com" target="_blank" rel="noopener noreferrer">
          <LetterboxdLogo className={styles.logoImg} />
        </a>
        <span>Diary</span>
        {layout === "carousel" && totalPages > 1 && (
          <div className={styles.carouselHeaderNav}>
            <button
      ...
      {username && (
        <div className={styles.footer}>
          view {name}&apos;s full letterboxd profile @{" "}
          <a
